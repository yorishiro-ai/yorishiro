...
| `YORISHIRO_DATABASE_DRIVER` | PostgreSQL only on this binary: unset (the default) or explicitly `postgres` both work, and any other value, including `sqlite`, refuses to start outright. Billing, OAuth, and tenant-scoped keys are all Postgres-concrete, so this binary never runs against Sqlite. A Sqlite deployment needs the community binary (`yorishiro-ce-server`) instead; see [the main configuration reference](../../docs/configuration.md#core) for what that one accepts |
