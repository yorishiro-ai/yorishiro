...
Served by **this** edition.
It was part of the community edition until base v0.42.0, which removed it: distributing templates between tenants is an enterprise capability whatever table stores them.
The paths and payloads are unchanged from then, so a client written against the old endpoints needs no edit.
...
Served by **this** edition since base v0.42.0, for the same reason as the marketplace: distributing a template's changes to its copies is template distribution.
**Creating** a schema from a template is not, and stayed in the community edition — `template_id` on `POST /api/schemas`, `GET /api/templates`, and the `origin_*` columns are untouched.
...
Served by **this** edition since base v0.44.0.
It was built in the community edition first, on the reading that a bring-your-own-key design costs the deployment nothing and could therefore live there.
That tested the wrong property: **what decides an edition is that the server makes an outbound chat completion at all**, not who pays for the call.
...
A name that resolved to a public address at `PUT` can resolve to `127.0.0.1` by the time the request goes out, so a check at storage time validates a different resolution than the one used — the reqwest seam for this is a custom DNS resolver, with any storage-time check kept only as an error message a person sees sooner.
...
1. Looks up an existing Yorishiro user by `(provider, subject_id)` -- the ID token's `sub` claim, not its `email`, since `sub` is what a provider actually guarantees stable and unique.
   This lookup does not require the ID token's `email_verified` claim to be `true`; an unverified email is still accepted for auto-provisioning (a warning is logged, but the login proceeds) -- an identity provider that never verifies email addresses is trusting the provider's account-creation flow, not this server, to prevent someone from claiming an address they don't own.
2. On first login, auto-provisions a new user (keyed to the ID token's `email` claim), a new tenant named after the email's local part, a schema from the built-in `general-notes` template, and a default workspace linked to that schema (capped at the Free plan's entity limit, since a freshly auto-provisioned tenant has no Stripe subscription yet), plus a `member`-role membership.
   ...
   The tenant/schema/workspace created earlier in the same attempt are not part of that transaction and may still exist after such a crash -- harmless, since nothing looks a tenant up by "was it ever joined" -- so a retry only guarantees no orphaned *user*, not a fully clean slate.
