...
| `YORISHIRO_DATABASE_DRIVER` | このバイナリはPostgreSQL専用です。未設定(既定)または明示的に`postgres`のどちらでも動作し、`sqlite`を含むそれ以外の値は起動そのものを拒否します。課金・OAuth・テナントスコープキーはいずれもPostgres前提の設計であり、このバイナリがSqliteで動くことはありません。Sqliteデプロイメントにはコミュニティ版バイナリ(`yorishiro-ce-server`)が必要です。コミュニティ版が受け付ける値は[設定リファレンス本体](../../../docs/ja/configuration.md#基本)を参照 |
