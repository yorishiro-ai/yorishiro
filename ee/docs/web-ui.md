...
Authenticated data requests go through the same bearer-key REST API documented in the public repo's `docs/api.md` and this repo's [api.md](api.md); the SPA carries no privileged access the API itself doesn't already enforce.
...
| `/api-keys` | Shows the current session's identity (workspace/tenant/user ID/scope) and admin-CLI reference commands (`create-api-key`, `list-api-keys`, `revoke-api-key`); actual key issuance/revocation/listing has no REST endpoint and is done via those `yorishiro-hosted-server admin` subcommands, not this page |
...
The SPA sends that header when — and only when — the URL names a workspace other than the key's own, which it records at sign-in.
