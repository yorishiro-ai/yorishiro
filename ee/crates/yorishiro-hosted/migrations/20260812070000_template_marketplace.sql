-- Template marketplace, moved here from the community edition.
--
-- The DDL is copied verbatim from base's `20260811090000_template_marketplace.sql` so the two
-- cannot drift, with `IF NOT EXISTS` added. That guard is what makes all three database states
-- work with one file:
--
--   * an existing deployment already has these tables through the vendored base migration,
--     which is still applied -- this is a no-op there;
--   * a fresh database created before base recreates its migrations gets them from vendor
--     first, then no-ops here;
--   * a fresh database created after that recreation (where base no longer creates them) gets
--     them here, which is the only remaining source.
--
-- Base kept its own file rather than deleting it, because `sqlx` validates applied migrations
-- against the embedded set and a file that vanishes fails every boot with `VersionMissing`.
-- The hosted binary is safe from the reverse problem too: `run_migrations` passes
-- `set_ignore_missing(true)` on both the vendored and the local set.
