...
//!
//! Distributing a template's later edits into the schemas copied from it is an enterprise capability, so this lives here rather than in the community edition.
//! Creating a schema from a template is *not* part of it: base owns `template_id` on `POST /api/schemas` and the `origin_*` columns.
