...
//! is control-plane data the request role holds no grant on. Passing the identity pool for the
//! schema side would bypass RLS; passing the scoped connection for the template side would fail.
...
async fn merge_sides(
    ...
    // written against the current active version would find their schema replaced by an older
    // lineage. Refuse instead, and name the schema so the caller can look up the active version.
