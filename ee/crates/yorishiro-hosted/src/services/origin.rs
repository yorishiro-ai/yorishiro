...
//! Ported from master's `ee/crates/yorishiro-hosted/src/services/origin.rs`.
//!
...
//! afterwards.
//!
//! These functions take both a schema connection and `ctx`, and the split is not incidental: the
//! schema is workspace content, read over the RLS-scoped connection (the one an `Authorized`
//! extractor's transaction holds), while `identity_templates` is control-plane data the request
//! role holds no grant on and can only be reached through `ctx.db`. Passing `ctx.db` for the
//! schema side would bypass RLS; passing the RLS-scoped connection for the template side would
...
/// current template for the missing base would read every local addition as a conflict, which is
...
async fn merge_sides(
    ...
    // definition. Merging into one is a different matter: `create_schema` archives whatever is
    // currently active and installs the result as the new active version, so merging an archived
    // version would resurrect an abandoned definition as the live one, and entities written
    // against the current active version would find their schema replaced by an older lineage.
...
///
/// Refuses a merge with conflicts, for the reason [`merge::apply_plan`] gives: a partially
/// applied merge is a definition neither side asked for.
///
