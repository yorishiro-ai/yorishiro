...
//! The pure reads (`list_marketplace`, `list_versions`, `list_reviews`) have no decision attached and live in `models::marketplace` alone; `http::controllers::marketplace` calls them directly.
...
pub async fn publish_version(
    ...
    // `UNIQUE (template_id, version)` catches it, which is why this was never corruption, but the
    // loser got an opaque 500 for doing nothing wrong.
    //
    ...
    let record = marketplace::insert_next_version(&mut tx, template_id, &request, user_id).await?;
...
pub async fn submit_review(
    ...
    if !marketplace::is_visible(pool, tenant_id, template_id).await? {
        return Err(YorishiroError::not_found(format!(
            "template '{template_id}' was not found"
        )));
    }

    marketplace::upsert_review(pool, tenant_id, template_id, user_id, &request).await
}
...
pub async fn fork_template(
    ...
    let Some(source) = marketplace::find_fork_source(pool, tenant_id, template_id).await? else {
        return Err(YorishiroError::not_found(format!(
            "template '{template_id}' was not found"
        )));
    };
    ...
    let Some(definition) =
        marketplace::find_forkable_definition(pool, template_id, version).await?
    else {
    ...
    match marketplace::insert_fork(pool, tenant_id, &source, &definition, template_id, user_id)
        .await?
    {
...
pub async fn set_visibility(
    ...
    marketplace::update_visibility(pool, template_id, visibility).await
}
...
async fn require_ownership(
    ...
    if !marketplace::is_owned_by(pool, tenant_id, template_id).await? {
        return Err(YorishiroError::not_found(format!(
            "template '{template_id}' was not found"
        )));
    }
