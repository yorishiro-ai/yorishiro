//! ID token (JWT) verification, per OpenID Connect Core §3.1.3.7. Signature verification uses
//! whichever key in the provider's JWKS matches the token's `kid` header; `iss`/`aud`/`exp` are
//! checked by `jsonwebtoken`'s own `Validation`.
...
/// The claims this integration actually needs. OIDC providers may include many more; anything
/// else is ignored by `serde`'s default (non-`deny_unknown_fields`) behavior.
...
pub fn verify(
    ...
    // Auth0 all sign with it by default); restricting to it also closes off the classic "alg:
    // none" / algorithm-confusion class of JWT vulnerabilities by construction, rather than by
    // remembering to check `header.alg` separately.
