...
//! Ported from master's `ee/crates/yorishiro-hosted/src/services/oauth/state_token.rs`.
//!
...
//! So instead of storing the PKCE code verifier server-side and looking it up by an opaque id
//! on callback, it is packed into the `state` value itself and HMAC-SHA256 signed via
//! [`crate::services::hmac_sign`] so the callback can trust whatever comes back without needing
//! to have remembered it.
//!
//! The signature alone only proves this process issued *some* `state` at some point: it says
//! nothing about whether the browser presenting it is the one the flow was started for.
//! That is what the CSRF cookie is for: `authorize` sets a random, per-browser value as an
//! `HttpOnly`/`Secure`/`SameSite=Lax` cookie and embeds `SHA256(cookie value)` in the signed
//! `state` payload; `callback` recomputes that hash from whatever cookie the browser actually
//! presents and rejects the request if it does not match [`verify`]'s `csrf_hash` output.
//! An attacker who captures a victim's `code`/`state` pair (by, say, starting their own login
//! and relaying the callback URL) cannot forge the victim's browser's cookie, so the
//! double-submit check fails even though the `state` signature itself is valid.
...
/// Bounds how long an authorization-code flow can stay in flight, mainly to limit the window a
/// captured (but not yet used) `state`/redirect URL could be replayed in.
/// Also used as the CSRF cookie's max-age, so the cookie never outlives the `state` that depends
/// on it.
pub const STATE_TTL_SECS: i64 = 600;

/// Number of random bytes in the CSRF cookie value.
/// Only its SHA-256 hash is ever embedded in `state`, so this can be shorter than a value that
/// needed to resist offline brute-forcing on its own: 16 bytes (128 bits) is already far beyond
/// what onlookers could guess before the cookie expires.
...
pub struct IssuedState {
    ...
    /// Carries the PKCE code verifier too, so the callback can recover it via [`verify`]
...
/// CSRF hash it carries.
/// Rejects anything that does not parse, does not verify, or has aged past [`STATE_TTL_SECS`]:
/// each is indistinguishable from the others to the caller, so a forged, expired or malformed
/// `state` cannot be told apart by an attacker probing the endpoint.
///
/// This alone does **not** prove the presenting browser is the one the flow was started for.
/// See the module docs.
/// Callers must separately check the returned `csrf_hash` against the SHA-256 hash of the
/// browser's CSRF cookie.
...
mod tests {
    ...
    fn verify_rejects_a_tampered_payload() {
        let issued = issue(KEY);
        // Flip the last character of the payload (still inside the signed portion) without
        // re-signing: the signature no longer matches.
    ...
    fn verify_rejects_an_expired_state() {
        // Hand-builds a state the way `issue` would have `STATE_TTL_SECS + 1` seconds ago,
