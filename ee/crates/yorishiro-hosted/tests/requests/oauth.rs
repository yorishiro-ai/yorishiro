//! Covers what is reachable without a live identity provider: the unconfigured/configured
//! `status` shape, the `authorize`/`callback` unconfigured `404`, an unreachable issuer failing
//! loudly rather than redirecting, `callback`'s rejection paths (provider error, missing
//! `code`/`state`, a badly-signed `state`, an expired `state`), and `find_or_create`'s
//! provisioning rules called directly against `ctx.db` (the tenant cap, matching how
//! `tests/requests/stripe.rs` calls `billing::` functions directly alongside HTTP requests). The
//! redirect `authorize` builds on a *reachable* issuer, the CSRF cookie it sets, and a full
//! authorization-code round trip through `callback` all need a real or mocked IdP and are not
//! covered here.
...
/// A loopback address nothing listens on, so `authorize`'s discovery fetch fails fast with a
/// connection refusal rather than depending on real DNS/network reachability in CI. Loopback,
/// not a public hostname, so `discovery::require_https_or_loopback`'s `http://` exemption
/// applies without needing a TLS-terminating stub server.
const ISSUER_URL: &str = "http://127.0.0.1:1";
const CLIENT_ID: &str = "test-client";
const CLIENT_SECRET: &str = "test-client-secret";

/// A signed `state` value in the exact shape `services::oauth::state_token::issue` produces:
/// this crate keeps that module private, so a test that needs a validly-signed fixture (rather
/// than exercising the rejection paths, which need no valid signature at all) builds one the
/// same way, HMAC-SHA256 under the client secret.
...
/// `OAuthConfig::from_env` reads process env vars directly on every request (this branch has no
/// DI seam for it, see `controllers::oauth`'s module doc comment), so tests configure OAuth the
/// same way production does: by setting the vars for the duration of the request. `#[serial]` on
/// every test in this suite makes that safe.
...
/// An issuer set with no `client_id`/`client_secret` is a misconfiguration, not "unconfigured":
/// `OAuthConfig::from_env` used to panic here (it read once at boot, so a panic was a fail-fast
/// startup failure); read per-request instead, it must report `500` rather than crash the task
/// handling the request, and must not be silently treated as `enabled: false`.
#[tokio::test]
#[serial]
...
/// With no `YORISHIRO_OAUTH_ISSUER_URL` set, `authorize` and `callback` must answer exactly the
/// same `404` a route that does not exist would: a prober must not be able to tell "unconfigured"
/// apart from "absent".
#[tokio::test]
#[serial]
...
/// `authorize` reaches the identity provider to fetch its discovery document before it can
/// redirect anywhere, so a configured but unreachable issuer fails loudly with an internal error
/// rather than a redirect to nowhere. Exercising the redirect itself, and the CSRF cookie it
/// sets, needs a live IdP and is out of scope here (see the module doc comment).
#[tokio::test]
#[serial]
...
/// The provider itself reporting an error (the user declined consent, an app misconfiguration,
/// ...) redirects to the login page's failure state rather than surfacing raw JSON to a browser
/// mid-redirect, and clears the CSRF cookie so it is never reusable across two attempts.
#[tokio::test]
#[serial]
...
/// A forged `state` (never issued by this process, so its HMAC signature cannot verify) is
/// rejected with `401` before any provider is contacted: `handle_callback` checks `state` first.
#[tokio::test]
#[serial]
...
/// A `state` value signed correctly (under the deployment's own client secret, so the HMAC
/// check that `callback_rejects_a_state_with_a_bad_signature` covers passes) but too old (older
/// than `STATE_TTL_SECS`) is rejected the same `401` way: `state_token::verify`'s expiry check
/// runs before the CSRF cookie is ever consulted.
#[tokio::test]
#[serial]
...
/// `find_or_create` must not be a backdoor around `YORISHIRO_MAX_TENANTS`: with the cap already
/// met, a brand-new identity (one `find_by_oauth_identity` finds nothing for) is refused rather
/// than silently given a fresh tenant. Called directly against `ctx.db` rather than through
/// `callback`, since driving this via HTTP would need a live IdP.
#[tokio::test]
#[serial]
