...
/// A workspace with nothing configured must read as absent, not as an error and not as an empty configuration: the caller turns absence into a refusal, and an empty `base_url` would instead produce a request to nowhere.
...
/// `describe` is what an endpoint returns, so the key must not be in it.
/// `get` is the only path that yields the key, and only the inference client calls it.
#[sqlx::test(migrations = "../../../migrations")]
...
/// Reconfiguring replaces rather than accumulating: a workspace has one set of credentials, and a second row would leave which one is used up to row order.
#[sqlx::test(migrations = "../../../migrations")]
...
/// A trailing slash on the base URL would produce `…/v1//chat/completions`.
/// Some providers accept that and some 404, so it is normalized once here rather than at each call site.
#[sqlx::test(migrations = "../../../migrations")]
...
/// An empty key is a configuration mistake that would otherwise be stored and fail later, at the provider, as a 401 the operator has to trace back here.
#[sqlx::test(migrations = "../../../migrations")]
...
/// Both directions, because a check that only ever sees valid input proves nothing about what it rejects.
/// `file://` is the case worth naming: it would make reqwest do something that is not an HTTP conversation, with a workspace's key attached.
#[test]
...
/// `check_scheme` alone did not catch this: it trims before looking, so it accepts padding that `set` was then storing verbatim.
/// The stored value is interpolated into a request URL, so what matters is what lands in the row, which only a `set`-level test can see.
#[sqlx::test(migrations = "../../../migrations")]
