...
/// The `Secure` cookie attribute is derived from the redirect URI's scheme rather than a separate variable.
/// Getting this backwards breaks the flow in a way that is hard to diagnose:
/// a `Secure` cookie is never returned to a plain-http callback, so local testing would fail with a CSRF mismatch rather than an obvious error.
#[test]
...
/// A missing or empty required variable must fail loudly and name itself.
/// Silently accepting an empty client secret would produce an OAuth flow that fails only at the token exchange, far from the cause.
#[test]
...
/// The default redirect URI has to be reachable from a browser.
/// An unspecified bind address (`0.0.0.0`) is not, so it is rewritten to a loopback host.
#[test]
fn an_unspecified_bind_address_is_rewritten_to_a_reachable_host() {
    ...
    // A concrete address is already reachable and is left alone, as is anything that is not a socket address at all (a hostname the operator wrote themselves).
    assert_eq!(
