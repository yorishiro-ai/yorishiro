...
fn round_trips_a_freshly_issued_state() {
    ...
    // The recovered verifier must hash to the challenge that was sent to the provider at authorize time: that's the actual PKCE property this round trip needs to preserve.
...
/// Signs a `state` payload as [`issue`] would, but with a caller-chosen issue time.
/// The signature is genuine, so anything rejected here is rejected on age alone, not because the HMAC failed.
...
/// The TTL is what bounds how long a captured `state` stays replayable.
/// Without this, a correctly-signed `state` would be accepted forever and the constant would be decoration.
#[test]
...
/// A future-dated `state` cannot have been issued by this process, so accepting one would mean a signing key leak had produced a token that outlives every TTL check.
/// A few seconds of skew are allowed because the issuing and verifying instances are different machines.
#[test]
...
/// `splitn(4, '.')` puts everything after the third dot into the signature field, so a payload carrying an extra dot would silently shift the parts.
/// The explicit `parts.next().is_some()` guard rejects it instead; this pins that the guard stays.
#[test]
...
/// A non-numeric timestamp verifies as a signature but cannot be aged.
/// It has to be rejected rather than treated as time zero, which would make it permanently valid.
#[test]
