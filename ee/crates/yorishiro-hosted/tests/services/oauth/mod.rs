...
fn url_with_query_errors_instead_of_panicking_on_a_malformed_base() {
    // A misbehaving/misconfigured provider's discovery document is untrusted, external input: this must never panic the request-handling task (see the function's doc comment).
