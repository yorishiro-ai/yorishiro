...
/// Writes a model's already-resolved answers for one entity straight to `entity_entities`, snapshotting first so `POST /api/migration-jobs/{job_id}/undo` can reverse it.
...
pub async fn apply_answers(
    ...
    match crate::models::entity_entities::update(conn, workspace_id, entity.id, data, None).await {
        ...
        Err(YorishiroError::NotFound { .. } | YorishiroError::ValidationFailed { .. }) => {
            ...
            crate::models::entity_entities::delete_snapshot(conn, workspace_id, entity.id, job_id)
                .await?;
            Ok(false)
        }
        Err(err) => Err(err),
