...
use crate::models::_entities::content_entities as content_entities_entity;
use crate::models::_entities::content_schemas;
...
pub async fn apply_answers(
    ...
    crate::models::content_entities::snapshot(conn, workspace_id, entity.id, job_id).await?;

    match crate::models::content_entities::update(conn, workspace_id, entity.id, data, None).await {
        Ok(_) => Ok(true),
        Err(YorishiroError::NotFound { .. } | YorishiroError::ValidationFailed { .. }) => {
            ...
            crate::models::content_entities::delete_snapshot(conn, workspace_id, entity.id, job_id)
                .await?;
            Ok(false)
        }
        Err(err) => Err(err),
