...
Vector search and full-text search are available on SQLite via `sqlite-vec` and FTS5 respectively. `sqlite-vec` is registered as an auto-extension in `src/db.rs::register_sqlite_extensions` (mirroring `main.rs`), so every connection opened on a SQLite URL automatically loads vector functions (`vec_distance_cosine`, etc.). `content_entity_embeddings` stores vectors as `BLOB` on SQLite and `vector(768)` on PostgreSQL; the migration crate creates both backends' versions via `helpers::pg_only` / `sqlite_only`.
...
`tests/` does include six dedicated SQLite test files (`content_entities_sqlite.rs`, `search.rs`'s FTS5 fallback, `tenancy_sqlite.rs`, plus three request-test files: `schemas_sqlite.rs`, `workspaces_sqlite.rs`, and `auth_sqlite.rs`), each gated by `require_sqlite_backend()`.
