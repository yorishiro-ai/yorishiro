...
Vector search on SQLite is available via `sqlite-vec`, registered as an auto-extension in `src/db.rs::register_sqlite_extensions` (mirroring `main.rs`), so every connection opened on a SQLite URL automatically loads vector functions (`vec_distance_cosine`, etc.). `content_entity_embeddings` stores vectors as `BLOB` on SQLite and `vector(768)` on PostgreSQL; the migration crate creates both backends' versions via `helpers::pg_only` / `sqlite_only`. Full-text fallback on SQLite is a plain `LIKE` against `content_entities.data`, not FTS5: UUID columns are `BLOB` on that backend (matching SeaORM's own bind format for `Uuid`), so the entity API's `.filter()` works directly with no hex-string conversion, and no separate FTS mirror table is needed.
