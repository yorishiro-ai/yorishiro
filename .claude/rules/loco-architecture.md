...
Vector search (pgvector/HNSW) and full-text search (`pg_trgm`) are not ported to SQLite yet — `content_entities` has no `embedding` column at all on that backend — see `docs/sqlite.md`.
