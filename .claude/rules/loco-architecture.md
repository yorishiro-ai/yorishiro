...
`create_tenant` also sets `identity_tenants::ActiveModel::id` itself on the SQLite branch only (`Uuid::now_v7()`, `uuid`'s `"v7"` feature), since that column's `uuidv7()` default is Postgres-only.
Every other `ActiveModel` insert path has the same gap and is unaddressed; the general fix belongs with the SQLite connection path in `src/db.rs` when that lands.
