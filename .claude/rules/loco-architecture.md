...
`embedding_keys::set` (`ee/models/embedding_keys.rs`) checks a new provider's `dimensions` against the workspace's own stamped `identity_workspaces.embedding_dimensions` at assignment time, refusing a mismatch with `ValidationFailed` before anything is stored; this sits in front of `sync_embedding`'s write-time guard (`src/services/embedding/sync.rs`) rather than replacing it, since a workspace created before either check existed, or one assigned by direct database access outside this endpoint, still needs the write-time check.
