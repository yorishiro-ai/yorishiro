...
Run `make entities`: it spins up a disposable `pgvector/pgvector:pg18` container, runs migrations, generates entities, and tears everything down — no `DATABASE_URL` needed and no risk of pointing at a stale or partial schema.
...
Vector search on SQLite is available via `sqlite-vec`, registered as an auto-extension in `src/db.rs::register_sqlite_extensions` (mirroring `main.rs`), so every connection opened on a SQLite URL automatically loads vector functions (`vec_distance_cosine`, etc.). `content_entity_embeddings` stores vectors as `BLOB` on SQLite and `vector(768)` on PostgreSQL; the migration crate creates both backends' versions via `helpers::pg_only` / `sqlite_only`. KNN search on the BLOB column uses the `vec0` virtual table (`content_entity_embeddings_vec0`), which exposes `MATCH`/`k=` operators without copying data. Full-text fallback on SQLite uses the `content_entities_fts` FTS5 virtual table with `tokenize='trigram'` for character-n-gram fuzzy matching (the same semantic approach as PostgreSQL pg_trgm); `entity_id` is an explicit column so the join back to `content_entities` never depends on rowids. UUID columns remain `BLOB` on that backend, so entity joins work without hex-string conversion.
...
`tests/models/search.rs` also contains a SQLite-specific test (`search_by_vector_falls_back_to_fts5_on_sqlite`), gated by `require_sqlite_backend()`.
...
`tests/models/search.rs` covers both the vector path and the FTS5 trigram fallback on SQLite, each gated by `require_sqlite_backend()`.
