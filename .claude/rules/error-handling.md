...
- Use `crate::ResultExt` (`.internal()`) for any fallible call that produces a non-`YorishiroError` error.
  ...
  `services/embedding/local.rs` has a private `fn internal(message: impl Display)` for exactly that case: a local helper like it is the sanctioned pattern when a module needs it repeatedly.
