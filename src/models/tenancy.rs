...
use crate::models::_entities::{invites, tenant_memberships, tenants, users, workspaces};
...
pub async fn count_tenants(conn: &impl ConnectionTrait) -> Result<u64, YorishiroError> {
    tenants::Entity::find()
        .filter(tenants::Column::Id.ne(INFRASTRUCTURE_TENANT_ID))
        .count(conn)
        .await
        .internal()
}
...
pub async fn create_tenant(
    ...
    name: &str,
) -> Result<tenants::Model, YorishiroError> {
    ...
    // `id` on SQLite is filled in by tenants::ActiveModel's before_save (crate::db::sqlite_generated_id), not here.
...
) -> Result<users::Model, YorishiroError> {
    let password_hash =
...
pub async fn verify_login(
    ...
    email: &str,
    password: &str,
) -> Result<Option<users::Model>, YorishiroError> {
...
pub async fn create_invite(
    ...
    role: MembershipRole,
    ttl: Duration,
) -> Result<(invites::Model, String), YorishiroError> {
...
pub async fn redeem_invite(
    ...
    let invite = invites::Entity::find()
        .filter(invites::Column::TokenHash.eq(token_hash.clone()))
        .filter(invites::Column::UsedAt.is_null())
        .filter(invites::Column::ExpiresAt.gt(now))
        .one(conn)
        .await
        .internal()?;
    ...
    let update_result = invites::Entity::update_many()
        .col_expr(
            invites::Column::UsedAt,
            sea_orm::sea_query::Expr::value(now),
        )
        .filter(invites::Column::Id.eq(invite.id))
        .filter(invites::Column::UsedAt.is_null())
        .filter(invites::Column::ExpiresAt.gt(now))
        .exec(conn)
        .await
...
pub async fn list_workspaces_for_user(
    ...
    let workspaces = workspaces::Entity::find()
        .filter(workspaces::Column::TenantId.is_in(tenant_ids))
        .all(conn)
        .await
        .internal()?;
...
pub async fn create_workspace(
...
) -> Result<workspaces::Model, YorishiroError> {
    use crate::models::_entities::tenants;
    use crate::models::workspaces::{WORKSPACE_STATUS_ACTIVE, WORKSPACE_STATUS_SCHEMA_PENDING};
    ...
    if let Some(max) = tenant.max_workspaces {
        crate::db::lock_for_update(conn, &workspace_count_lock_key(tenant_id))
            .await
            .internal()?;
        let count = workspaces::Entity::find()
            .filter(workspaces::Column::TenantId.eq(tenant_id))
            .count(conn)
            .await
            .internal()?;
    ...
    let active = workspaces::ActiveModel {
        tenant_id: ActiveValue::Set(tenant_id),
        name: ActiveValue::Set(name.to_string()),
...
pub async fn delete_workspace(
    ...
    let remaining = workspaces::Entity::find()
        .filter(workspaces::Column::TenantId.eq(workspace.tenant_id))
        .count(conn)
        .await
        .internal()?;
    if remaining <= 1 {
