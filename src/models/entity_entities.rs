...
pub async fn drift(
    ...
    let own = super::schema_schemas::get_by_id(conn, workspace_id, entity.schema_id).await?;
    let active = super::schema_schemas::get_active_schema(conn, workspace_id, &own.name).await?;
...
pub async fn migration_dry_run(
    ...
    for row in rows {
        ...
        let old = match definitions.get(&row.schema_id) {
            ...
            None => {
                let record =
                    super::schema_schemas::get_by_id(conn, workspace_id, row.schema_id).await?;
...
pub async fn delete_snapshot(
    ...
    use super::_entities::entity_snapshots;
