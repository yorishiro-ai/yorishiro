...
/// Summary of a built-in schema template, returned by `list_templates` so a caller can pick a
/// `template_id` without first fetching every template's full definition.
