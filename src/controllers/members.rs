...
/// stricter than) the presented API key's own scope.
...
pub struct AddMemberRequest {
    ...
    /// *existing* user to the caller's tenant, it never creates one. To bring in someone with no
    /// account yet, issue them an invite instead.
    pub email: String,
