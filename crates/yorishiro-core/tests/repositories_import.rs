...
use yorishiro_core::repositories::export::export_all;
use yorishiro_core::repositories::import::import_jsonl;
...
fn to_jsonl(records: &[yorishiro_core::repositories::export::ExportRecord]) -> String {
...
async fn imports_schema_entities_and_relations_from_jsonl(pool: PgPool) {
    ...
    let imported_relation = dest_records
        .iter()
        .find_map(|r| match r {
            yorishiro_core::repositories::export::ExportRecord::Relation(rel) => Some(rel),
            _ => None,
        })
    ...
    let imported_entity_ids: Vec<Uuid> = dest_records
        ...
        .filter_map(|r| match r {
            yorishiro_core::repositories::export::ExportRecord::Entity(e) => Some(e.id),
            _ => None,
        })
        .collect();
