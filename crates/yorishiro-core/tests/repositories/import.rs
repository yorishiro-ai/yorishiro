...
use crate::repositories::export::export_all;
use crate::repositories::import::import_jsonl;
use crate::repositories::relations::CreateRelationInput;
use crate::repositories::{entities, relations, schemas};
...
async fn imports_schema_entities_and_relations_from_jsonl(pool: PgPool) {
    ...
    let imported_relation = dest_records
        .iter()
        .find_map(|r| match r {
            crate::repositories::export::ExportRecord::Relation(rel) => Some(rel),
            _ => None,
        })
    ...
    let imported_entity_ids: Vec<Uuid> = dest_records
        ...
        .filter_map(|r| match r {
            crate::repositories::export::ExportRecord::Entity(e) => Some(e.id),
            _ => None,
        })
        .collect();
