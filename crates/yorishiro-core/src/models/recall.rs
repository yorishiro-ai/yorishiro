...
/// dense graphs (each additional hop can multiply the number of neighbor fetches).
...
pub struct RecallRelation {
    ...
    /// How many hops away from the requested entity this neighbor is (1 = direct neighbor,
    /// 2 = neighbor-of-neighbor, etc).
...
pub struct RecallContext {
    ...
    /// `hop_distance`. A neighbor reachable via more than one path is only reported once, at
    /// the shortest hop_distance it was found at.
...
pub struct RecallQuery {
    ...
    /// `[1, MAX_RECALL_DEPTH]`; defaults to `DEFAULT_RECALL_DEPTH` (1, the original
    /// single-hop behavior).
    pub depth: i64,
}
