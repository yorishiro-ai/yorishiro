...
///
/// Asymmetric models (Qwen3-Embedding among them) expect a search query to carry an instruction prefix that a stored document must not have.
...
/// The `entities.embedding` column is dimensionless (`vector`), so any model works.
/// All vectors in a deployment must share the same dimension count (`YORISHIRO_EMBEDDING_DIMENSIONS`).
#[async_trait]
pub trait EmbeddingProvider: Send + Sync {
    ...
    ///
    /// The default ignores `kind` and delegates to [`Self::embed`], which is correct for every symmetric model, including this crate's defaults.
    /// A provider whose model treats queries and documents differently overrides this; one that does not needs no changes, which is why the method carries a default rather than being required.
    ///
    /// Callers say what the text is for and nothing more.
    /// What gets prepended, if anything, is the model's own convention and belongs to the provider.
