...
// `YSR_LOG_TARGET` is process-wide state, so these serialize through one lock rather than
...
/// by hand at the end would leak the value into whichever test ran next on an assertion failure.
struct TargetGuard {
...
impl TargetGuard {
    fn set(value: &str) -> Self {
        ...
        unsafe { std::env::set_var("YSR_LOG_TARGET", value) };
        Self { _lock: lock }
    }
}
...
impl Drop for TargetGuard {
    fn drop(&mut self) {
        // SAFETY: serialized by ENV_LOCK, and nothing else touches this key.
        unsafe { std::env::remove_var("YSR_LOG_TARGET") };
    }
}

/// A typo in `YSR_LOG_TARGET` has to stop startup. Falling back to a default would leave an
...
/// An empty value is a real shape -- `YSR_LOG_TARGET=` in a compose file or `.env` -- and must
...
fn an_empty_target_is_rejected_rather_than_treated_as_unset() {
    ...
    assert!(
        init().is_err(),
        "an empty YSR_LOG_TARGET should be rejected, not silently treated as the default"
    );
}

/// The default target needs no configuration, which is what makes the Docker image work with
/// no logging setup at all -- an unset `YSR_LOG_TARGET` must resolve to `stdout` rather than
/// being rejected like an unknown value.
///
/// This asserts the resolution, not `init()` itself: `init()` installs the global tracing
/// subscriber, which can only ever be set once per process, so calling it here would panic
/// whenever another test in the same binary got there first.
#[test]
fn an_unset_target_resolves_to_the_default() {
    ...
    let resolved = std::env::var("YSR_LOG_TARGET").unwrap_or_else(|_| "stdout".into());

    assert_eq!(
        resolved, "stdout",
        "an unset YSR_LOG_TARGET must fall back to stdout"
    );
}
