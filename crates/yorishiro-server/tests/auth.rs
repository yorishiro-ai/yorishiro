...
use yorishiro_server::build_app;
use yorishiro_server::test_support::*;
...
async fn login_requires_workspace_id_when_the_account_has_access_to_more_than_one(pool: PgPool) {
    ...
    // `yorishiro_server::max_tenants_env_lock` for why this is a shared, crate-wide lock.
    let _guard = yorishiro_server::max_tenants_env_lock::LOCK.lock().unwrap();
