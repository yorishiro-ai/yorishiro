...
/// The scope/role arguments are the CLI's surface for values the database constrains. Their
/// conversions are what stop `admin create-api-key <ws> write` from writing a value the column
/// rejects, so each arm is pinned.
#[test]
...
/// Same for roles: the CLI accepts four, and they must map onto the four the membership check
/// constraint permits.
#[test]
