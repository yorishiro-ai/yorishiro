...
/// `definition` and `template_id` are mutually exclusive and both optional at the type level, so deserialization accepts either.
/// The exclusivity is enforced by the handler, and this pins that the shape itself permits both spellings a client might send.
#[test]
