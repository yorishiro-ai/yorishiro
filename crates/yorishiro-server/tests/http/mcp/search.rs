...
/// Search is driven by the query text, which is the only required argument.
/// Everything else narrows an already-valid search.
#[test]
...
/// The filter is a JSONB containment object passed through to the query; it must survive as a structured value rather than being flattened to a string.
#[test]
