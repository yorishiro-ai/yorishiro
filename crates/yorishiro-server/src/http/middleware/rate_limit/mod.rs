...
/// A per-key fixed-window rate limiter, applied via [`apply_rate_limit_layer`] to whichever routes a caller considers reachable without a bearer token (in this crate, `/auth/signup`, `/auth/login`, `/setup`, `/setup/status`): the ones exposed to unauthenticated credential/invite-token brute-forcing.
/// A downstream crate adding its own unauthenticated routes (e.g. an OAuth callback) needs the same protection; see `apply_rate_limit_layer`'s doc comment.
/// Keyed by client IP; falls back to a single shared bucket when no `ConnectInfo` is present on the request (e.g. tests driven through `Router::oneshot`, which never populates it).
...
impl RateLimiter {
    ...
    /// `YORISHIRO_AUTH_RATE_LIMIT_MAX` (default 10) requests per `YORISHIRO_AUTH_RATE_LIMIT_WINDOW_SECS` (default 60) seconds, per client IP.
    ...
    ///
    /// `pub` (rather than private) solely so the crate-root `tests/` integration tests (which only see this crate's public API) can exercise the bucket logic directly instead of only through `enforce`.
...
/// Applies [`enforce`] to `router`, keyed by `limiter`.
/// Factored out so a process embedding this crate's routes alongside its own (e.g. a downstream crate adding an OAuth login/callback pair, which is exactly as reachable without a bearer token as `/auth/login`) can rate-limit its own unauthenticated routes the same way this crate does its own: `axum::Router::merge` doesn't propagate a `.layer()` from either side to the other, so each sub-router must carry its own copy.
///
/// Pass the *same* `Arc<RateLimiter>` used for this crate's own auth routes (for a downstream crate embedding [`crate::build_app`], reuse the limiter that produced its rate-limited routes: see that function's doc comment) to share one quota across both; pass a fresh `Arc::new(RateLimiter::from_env())` instead for an independent quota.
