...
///
/// Decided by method: `POST` to `/mcp` may be a read, but the middleware cannot know which
/// tool without parsing the body, and a body consumed here is a body the handler no longer
/// has. Read-only mode therefore refuses MCP wholesale, which errs toward refusing a read
...
pub async fn maintenance_guard(
    ...
    let mut conn = match state.identity_pool.acquire().await {
        Ok(conn) => conn,
        // The database being unreachable is not a reason to refuse: the handler is about to
        // fail on its own with an error that says so, which is more useful than a maintenance
        // notice nobody switched on.
