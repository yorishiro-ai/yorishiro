...
/// That is deliberate for the Postgres-only `templates.created_by` FK (`ON DELETE SET NULL`) and the `tags TEXT[]` column, both absent on SQLite by design.
...
impl MigrationTrait for Migration {
    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // No RLS and no GRANT for this table: users is a control-plane table reached only through the migration-role identity pool (see src/db.rs), never a tenant-scoped request connection, so yorishiro_app has no business touching it directly.
        ...
        // No GRANT: tenant_memberships is control-plane data, reached only through the migration-role identity pool (see src/db.rs), never a tenant-scoped request connection, same reasoning as tenants and users.
        ...
        let mut table = Table::create()
            ...
            .foreign_key(
                ForeignKey::create()
                    ...
                    .from(Alias::new("workspaces"), Alias::new("tenant_id"))
                    .to(Alias::new("tenants"), Alias::new("id"))
                    .on_delete(ForeignKeyAction::Cascade),
            )
            .to_owned();
            ...
            table = table
                .foreign_key(
                    ForeignKey::create()
                        ...
                        .from(Alias::new("workspaces"), Alias::new("schema_id"))
        ...
        helpers::grant(manager, "SELECT", "workspaces").await?;
        ...
        helpers::pg_only(
            manager,
            "GRANT UPDATE (status, schema_id) ON workspaces TO yorishiro_app;",
        )
        .await?;
        ...
        let table = Table::create()
            ...
            .foreign_key(
                ForeignKey::create()
                    ...
                    .from(Alias::new("invites"), Alias::new("tenant_id"))
                    .to(Alias::new("tenants"), Alias::new("id"))
                    .on_delete(ForeignKeyAction::Cascade),
            )
            .to_owned();
        ...
        // No GRANT: invites is control-plane, same as tenants, users and tenant_memberships.
                ...
                Table::create()
                    ...
                    .foreign_key(
                        ForeignKey::create()
                            ...
                            .from(Alias::new("templates"), Alias::new("tenant_id"))
                            .to(Alias::new("tenants"), Alias::new("id")),
                    )
                    ...
                    .foreign_key(
                        ForeignKey::create()
                            ...
                            .from(Alias::new("templates"), Alias::new("fork_of"))
                            .to(Alias::new("templates"), Alias::new("id"))
                            .on_delete(ForeignKeyAction::SetNull),
                    )
                    // FK to users: created_by SET NULL for PostgreSQL (a template outlives
                    ...
                    .foreign_key(
                        ForeignKey::create()
                            ...
                            .from(Alias::new("templates"), Alias::new("created_by"))
                            .to(Alias::new("users"), Alias::new("id"))
        ...
        manager
            .get_connection()
            .execute_unprepared("INSERT INTO maintenance (id) VALUES (TRUE);")
            .await?;
        ...
        helpers::grant(manager, "SELECT", "maintenance").await?;
        ...
        // The circular half: `workspaces.schema_id` is created as a plain column with no FK above, since content_schemas did not exist yet at that point.
        ...
        // fires on templates, matching the old DDL's own ordering choice of
        ...
        // No GRANT and RLS anyway, for the reason tenants gives above: this table is reached only through ctx.db, never a tenant-scoped request connection.
        helpers::enable_rls_with_policy(
            manager,
            "tenant_billing",
        ...
        manager
            .create_table(
                Table::create()
                    .table(Alias::new("stripe_processed_events"))
                    .if_not_exists()
                    .col(
        ...
        // yorishiro_app gets no GRANT at all here, matching tenants/tenant_billing: this table is reached only through ctx.db (the migration-role connection, the Stripe webhook handler's only DB access), never a tenant-scoped request connection.
        ...
        // No RLS, no GRANT to yorishiro_app on either table, matching templates itself: both are read in exactly the same paths (the repository layer, as the owner role, scoping by tenant in the query) and are never reached through the tenant pool.
        ...
        // No RLS and no GRANT, deliberately, matching templates: yorishiro_app is never the role that reaches this table.
        ...
        let table = Table::create()
            ...
            // SetNull, not the no-FK-at-all treatment api_key_id gets above: users rows are not routinely deleted the way a revoked key is, so a real FK here is cheap, and SetNull keeps a record of "some now-deleted user did this" rather than losing the row's referential integrity.
            .foreign_key(
                ForeignKey::create()
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        //
        // `workspaces.schema_id → content_schemas.id` and
        // `content_schemas.workspace_id → workspaces.id` form a circular FK pair.
        // `DROP TABLE workspaces CASCADE` breaks the cycle: CASCADE drops FK
        // constraints that reference `workspaces` (including `content_schemas`'s
        ...
        helpers::pg_only(
            manager,
            "DROP TRIGGER IF EXISTS templates_detach_schema_origins ON templates;
        ...
        for table in [
            ...
            "content_relations",                 // → workspaces, content_entities ×2
            "content_entity_snapshots",          // → workspaces, content_entities (no FK)
            "content_entity_column_preferences", // → workspaces
            "api_key_audit_log",                 // → workspaces, tenants, users
            "api_keys",                          // → workspaces, tenants, users
            "workspace_worker_classes",          // → workspaces
            "workspace_embedding_keys",          // → workspaces
            "workspace_llm_keys",                // → workspaces
            "content_entities",                  // → workspaces, content_schemas, users
            "content_schemas", // → tenants, workspaces, templates; dropped after content_entities
            "tenant_memberships", // → tenants, users
            "invites",         // → tenants
            "template_reviews", // → templates, tenants, users
            "template_versions", // → templates, users
            "templates",       // → tenants, templates(self-ref), users; must precede users
            "users",           // no FK deps (self-referential only); must follow templates
            "tenant_billing",  // → tenants
            "maintenance",     // no FK deps
            "tenants",         // no FK deps
        ] {
            manager
                .drop_table(
