...
async fn infer_fill_without_a_configured_key_is_refused() {
    ...
    boot_request::<App, _, _>(|request, ctx| async move {
        ...
        let infer = request
            ...
            .add_header("Authorization", format!("Bearer {}", setup.key))
            .await;
        assert_eq!(infer.status_code(), StatusCode::UNPROCESSABLE_ENTITY, "response: {:?}", infer.text());
    })
    .await;
}
...
async fn infer_job_status_is_on_its_own_path_not_colliding_with_infer_fill() {
    ...
    boot_request::<App, _, _>(|request, ctx| async move {
        ...
        let put_key = request
            ...
            .await;
        assert_eq!(put_key.status_code(), StatusCode::NO_CONTENT, "response: {:?}", put_key.text());
