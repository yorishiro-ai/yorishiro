...
async fn a_dimension_mismatch_against_the_workspace_stamp_is_refused_at_config_time() {
    ...
    boot_request::<App, _, _>(|request, ctx| async move {
        ...
        let mut active: workspaces::ActiveModel =
            workspaces::Entity::find_by_id(setup.workspace_id)
                .one(&ctx.db)
                .await
...
async fn setup_second_workspace(ctx: &loco_rs::app::AppContext, first: &Setup) -> Uuid {
    let first_workspace = workspaces::Entity::find_by_id(first.workspace_id)
        .one(&ctx.db)
        .await
