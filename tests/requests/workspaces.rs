...
async fn workspace_endpoints_enforce_tenant_isolation() {
    boot_request::<App, _, _>(|request, ctx| async move {
        ...
        // Tenant A's key must not be able to see or delete tenant B's workspace by guessing its id: workspaces has no RLS of its own, so get_workspace_in_tenant's explicit tenant_id check is the only thing enforcing this boundary.
        let response = request
