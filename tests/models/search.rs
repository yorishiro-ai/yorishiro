...
/// The FTS5 fallback on SQLite surfaces an entity whose `data` fuzzy-matches `query_text`, when
/// the entity has no embedding; an entity with neither an embedding nor a MATCH match must not
/// appear.
///
