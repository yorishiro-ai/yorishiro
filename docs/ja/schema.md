...
- テンプレートライブラリから作成したスキーマは、出自を`origin_template_id`に記録し`origin_status`は`"linked"`になる。**そのテンプレートを削除しても複製は壊れない**——リンクが切れて`"detached"`へ移るだけであり、利用中のワークスペースを壊さずにテンプレートを取り下げられる。手書きのスキーマは出自を持たないため最初から`"detached"`である。

## 互換な変更と破壊的な変更

スキーマを更新すると、新しい定義が現行の定義と差分比較される。互換な変更は既存バージョンを その場で更新し、破壊的な変更は新しいバージョンを作る。旧バージョンで書かれたレコードは それぞれのバージョンのまま読み出せる。
...
- 制約の厳格化: 制約が無かったところへ追加する、`minimum` / `minLength` / `minItems` を 引き上げる、`maximum` / `maxLength` / `maxItems` を引き下げる、`format` や `pattern` を 別の値へ変更する、`uniqueItems` を有効にする
