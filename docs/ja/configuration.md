...
REST APIでもリインデックスを登録できる。`POST /api/migration-jobs/reindex`(Migrationスコープ)はジョブIDを202で即時返し、リインデックスはバックグラウンドで非同期に走る。同じワークスペースに対する2つ目のリクエストは、1つ目がまだ動いていれば同じadvisory lockの待ちqueueに入る。APIは1つ目が残り何秒か予測できないので、2つ目を拒否できない。大規模ワークスペースのリインデックスは正しく長い時間を要する場合がある。キュープロバイダが設定されていない場合、503を返す。`perform_later`はキューが未設定でもjob IDを返すので、捏造IDを返して何も起きない状態より、エンドポイント自体が無い方がマシだからである。

起動時はPostgreSQL向けに自動リインデックス検知が走っている。ワークスペースの刻印済みモデルと現在のプロバイダを照合し、不一致のワークスペースには非ブロック型リインデックスを登録する。リインデックス中、そのワークスペースへのエンティティ書き込みは`409`(ReindexInProgress)で拒否されるが、既存の書き込み時モデルチェックがそのまま処理するので、新しい機構は不要である。
...
| 変数 | 説明 |
|---|---|
| `YORISHIRO_LOCAL_MODEL` | どのモデルを読み込むか。`multilingual-e5-base`。未設定の既定は`multilingual-e5-base`である。このコードベースの検索とrecallは英語専用ではないためである。認識できない値は、既定へ黙って倒れるのではなく、有効な値を名指ししたメッセージとともに起動を失敗させる。 |
...
`YORISHIRO_ONNX_MODEL_PATH`と`YORISHIRO_ONNX_TOKENIZER_PATH`はリネームされた変数で、メッセージは旧名と新名の両方を名指しする。
`YORISHIRO_LOCAL_MODEL_PATH`と`YORISHIRO_LOCAL_TOKENIZER_PATH`はさらにその後のリリースで完全に削除された変数である。運用者が選んだパスはモデル識別子(`YORISHIRO_LOCAL_MODEL`から来る)と構造的に結びついていなかったため、不一致を検知できず、分離そのものが廃止された。
`YORISHIRO_ONNX_MAX_SEQUENCE_LENGTH`は`YORISHIRO_LOCAL_MAX_SEQUENCE_LENGTH`へリネームされた。
...
いずれの場合も、起動をやり直す前に旧名を削除する(リネーム組については置き換え先の名前に変える)こと。

#### モデルの取得

モデルとトークナイザはリポジトリに入っていない。
モデル単体で、nomic-embed-text-v1.5は約522MiB、multilingual-e5-baseは約1.04GiBある。
既定の`models/<short_id>/`にファイルが無い場合、初回利用時に両方を`$HOME/.cache/yorishiro/models/<short_id>/`へ取得し、バイナリに埋め込んだSHA256で検証する。
...
`models/<short_id>/`に置いたファイルは一切検証しない。
任意に選んだモデルに対して固定できるダイジェストは存在せず、そもそも別のモデルを意図的に使っている可能性があるからである。
この信頼はバイト列だけでなく、モデルの識別にも及ぶ。このプロバイダは実際に読み込んだチェックポイントが何であれ、`YORISHIRO_LOCAL_MODEL`が名指しするカタログ上のモデルの`def.id`をそのまま報告する。そのため別のモデルの重みを置いた`nomic-embed-text-v1.5`用のチェックポイントも、本物のnomic-embed-text-v1.5であるかのようにワークスペースへ刻印され、書き込み時モデルチェックもそう扱って比較する。
