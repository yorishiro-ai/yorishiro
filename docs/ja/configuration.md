...
次回以降の起動では、ハッシュを取り直すのではなくキャッシュされたファイルのサイズを確認する。毎回522MiBを読み直せば、一度きりのはずのコストが恒久的なものになってしまうからである。
サイズが合わないファイルは削除して取得し直すので、途中で切れたり壊れたりしたキャッシュは、健全なものとして読み込まれるのではなく自力で復旧する。
