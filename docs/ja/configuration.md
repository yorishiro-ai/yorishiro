...
`nomic-ai/nomic-embed-text-v1.5`を`safetensors`形式のチェックポイントからプロセス内で実行し、外部の埋め込みサービスを使わない。
`candle-transformers`の`nomic_bert`モジュールがこのアーキテクチャ(RoPEとSwiGLU MLPを持つBERT亜種)専用の実装で、任意のエンコーダグラフを読めるわけではないため、このプロバイダは常にこの1モデルだけを読み込む。
プーリングは常にmeanプーリングである。
このモデルがそう学習されているうえ、このプロバイダが読み込めるアーキテクチャがこの1つしかない以上、プーリング方式を選ばせる余地自体が無い。
...
`YORISHIRO_ONNX_MODEL_PATH`・`YORISHIRO_ONNX_TOKENIZER_PATH`・`YORISHIRO_ONNX_MAX_SEQUENCE_LENGTH`はリネームされた変数で、メッセージは旧名と新名の両方を名指しする。これらを放置すると、このプロバイダの通常の解決処理(両方揃っていれば`models/`の既定ファイルを使い、両方無ければnomic-embed-text-v1.5を取得し、片方だけならエラーにする)が気づかれないまま走り、デプロイが思っているのとは別のモデルで静かに起動することになるためである。
...
`YORISHIRO_LOCAL_MODEL_PATH`と`YORISHIRO_LOCAL_TOKENIZER_PATH`のどちらも未設定で、かつ既定の`models/`にファイルが無い場合、初回利用時に両方を`$HOME/.cache/yorishiro/models/`へ取得し、バイナリに埋め込んだSHA256で検証する。
...
したがって次回以降の起動は、そこにあるものを「以前の検証済みダウンロードの結果」として扱い、毎回522MiBをハッシュし直すことはしない。
ただしサイズだけは確認する。
これは無償で、途中で切れたファイルを捕まえられるからである。
サイズが合わないファイルは削除して取得し直すので、その場合は健全なものとして読み込まれるのではなく自力で復旧する。
`models/`に置いたファイルやパス変数で指定したファイルは、一切検証しない。
