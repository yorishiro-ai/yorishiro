...
旧名を設定したままのデプロイは、旧名と新名の両方を名指ししたメッセージとともに起動に失敗する。
旧名はもう一切読まれない。特定のモデルを指していた`YORISHIRO_ONNX_MODEL_PATH`を放置すると、このプロバイダの通常の解決処理(両方揃っていれば`models/`の既定ファイルを使い、両方無ければnomic-embed-text-v1.5を取得し、片方だけならエラーにする)が気づかれないまま走り、デプロイが思っているのとは別のモデルで静かに起動することになる。起動をやり直す前に、旧名を削除するか置き換え先の名前にリネームすること。
