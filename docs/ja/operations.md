...
`split`は空白で区切り**語の途中では切らない**。`ChunkProgress`はACK済みのチャンクを記録し、
**未ACKのものを列挙する**。

**再割当はここには無い。** 未ACKのチャンクをプールへ戻すのは可視性タイムアウトであり、
それは**プロセス間で仕事を運ぶキューの性質**である。`LocalQueue`はこのランタイム上で
タスクを走らせるため、チャンクの喪失とプロセスの喪失が同じ事象になる。分散ドライバは
自前のタイムアウトを持ち込み、ここにあるものをそのまま使う——`outstanding()`が返すのは、
まさにそのドライバが戻すべき一覧である。
