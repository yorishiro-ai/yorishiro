...
2. デプロイメントに応じてデータベース・エンジン・版を用意します。

   - **エンタープライズ版**(`yorishiro-server`、既定のパッケージ): 常に**PostgreSQL 18以降**。
   - **コミュニティ版**(`yorishiro-ce-server`): **PostgreSQL 18以降**(このステップと次のロール設定はエンタープライズ版と同一に適用されます)、または代わりに**`sqlite`**。この場合は起動するだけなら書き込み可能なパス以外に何も用意する必要がありません。

   主キーに組み込みの`uuidv7()`を使っており、これはPostgreSQL 18で追加されたものです。
   それより古いサーバでは機能が縮退するのではなく、マイグレーション自体が失敗します。
   コミュニティ版バイナリでエンジンを選ぶ`YORISHIRO_DATABASE_DRIVER`については[configuration.md](configuration.md#基本)を参照してください。
   現時点で`sqlite`が対応するのは初回セットアップと認証のみです。
   REST APIの残り・MCP・下記の管理CLIはまだ配線されておらず、動くふりをせず明確に失敗します。
   それ以外の用途では引き続きPostgreSQLをデプロイしてください。
...
下記の管理CLIはコミュニティ版バイナリの`sqlite`エンジンでは利用できません。
動くふりをせず、エラーを示して明確に拒否します。
...
3. ログイン
   ...
   - この422の`details`には候補ワークスペースが1件ずつ入ります。
     `field`がワークスペースID、`problem`が名前なので、クライアントはこれでピッカーを描けます。
