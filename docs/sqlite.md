...
- **`content_entity_snapshots::snapshot`** (`INSERT ... SELECT` recording an entity's data before an overwrite) and **`undo_job`** (restoring from those snapshots): `snapshot` is raw, hardcoded-Postgres SQL like `neighbors_batch`; `undo_job` calls `ActiveModel::update(conn)` directly rather than going through `content_entities::update`, so it still hits the same return-value decode failure `create`/`update` used to.
  `POST /api/migration-jobs/{id}/undo` is blocked as a result.

Not blocked: entity CRUD (`POST`/`GET`/`PUT`/`DELETE /api/entities`), export/import (`GET /api/export.jsonl`, `POST /api/import.jsonl`), `GET /api/workspaces/{id}`'s `entity_count` field, and creating a relation (`POST /api/relations`, which calls `content_entities::get` on both endpoints).
