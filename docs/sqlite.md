...
So a caller sees a clear failure rather than "no entities": but the response body itself is a generic `500`, not a message naming SQLite specifically: a caller cannot yet distinguish "this backend doesn't support this route" from any other internal error without reading this document or the server's own log.
