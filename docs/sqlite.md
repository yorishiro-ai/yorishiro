...
`Verified<R>` deliberately has no SQLite branch: its one caller (`search_entities`) calls `db_handle()` directly regardless, but the handler body checks the backend first and returns 501 with a message naming the PostgreSQL requirement, since `content_entities.embedding` for vector similarity search does not exist on that backend (see "What's still blocked").
...
Measured with that guard bypassed, at `max_connections: 1` with `connect_timeout: 500`, which is what the guard exists to keep a deployment from reaching: a read-only route (`GET /api/relations`) still returned `200`, since the failed `last_used_at` update is best-effort and only logs a warning.
...
The SeaORM entity API builds its queries from every field on `Model` regardless of backend, so an unguarded query against this table fails on SQLite.
`count`, `get`, `get_batch`, `list`, `export_all`, `create`, `update`, and `delete` (`src/models/content_entities.rs`) therefore branch internally: on SQLite they query and decode a column list that excludes `embedding` (`select_record_columns`).
`create`/`update` route around a second, separate failure as well: `ActiveModelTrait::insert`/`update`'s decode of the returned row also touches `embedding`, and SeaORM's `pgvector::Vector` decode support unconditionally errors on any SQLite row regardless of whether the column exists.
The branches sit on the SQLite side only, so callers of these eight functions (`content_relations::create`, `controllers/workspaces.rs`'s `entity_count`, `recall.rs`, the entity CRUD/export/import routes) see the same input, return type and errors on both backends.
...
`POST /api/migration-jobs/{id}/undo` (`content_entities::undo_job`) works on SQLite.
It builds its own `ActiveModel` rather than going through `content_entities::update`, so it carries its own branch, using `active.update_without_returning(conn)` to avoid the same return-value decode `create`/`update` avoid.
Restoring from a snapshot works there once one exists; nothing in base itself writes one, since `snapshot` (above) remains `ee/`-only and Postgres-only.
...
The PostgreSQL schema those helpers produce (every table, column, constraint name, index, policy and grant) is exactly what a PostgreSQL-only migration would emit.
Some constraints reach it through `create_table_with_checks`/`pg_only` rather than a direct statement, and `identity_maintenance`'s three CHECKs are three separate `execute_unprepared` calls rather than one holding three semicolon-separated `ALTER TABLE` statements.
