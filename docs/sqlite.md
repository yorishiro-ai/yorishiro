...
- **`content_entity_snapshots::snapshot`** (`INSERT ... SELECT` recording an entity's data before an overwrite, called only from `ee/`'s fill-proposal batch confirmation): raw, hardcoded-Postgres SQL like `neighbors_batch`, unaffected by anything in this section.
