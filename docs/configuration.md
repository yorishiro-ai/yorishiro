...
Restarts with `YORISHIRO_LOCAL_MODEL` (or `YORISHIRO_EMBEDDING_MODEL`, for the OpenAI-compatible provider) already pointed at the new model, before any workspace is reindexed, is expected and correct, not a step to avoid.
...
You can also enqueue a reindex via the REST API. `POST /api/migration-jobs/reindex` (Migration scope) returns 202 with the job ID immediately and the reindex runs asynchronously in the background; the second request for the same workspace while the first is still running enqueues another job that waits on the same advisory lock. The API cannot reject the second request without knowing how long the first has left, and a reindex over a large workspace may legitimately take a long time. The endpoint returns 503 when no queue provider is configured: `perform_later` would silently return a job ID while discarding the job, which is worse than no endpoint at all.

A startup reindex detection runs automatically after boot on PostgreSQL. It checks every workspace's stamped model against the current provider and enqueues a non-blocking reindex for any mismatch. While the reindex runs, entity writes to that workspace are refused with `409` (`ReindexInProgress`), and the existing write-time model check handles this without any new mechanism.
...
`config/test_postgres.yaml` has no `queue:` block at all (`.claude/rules/testing.md` covers why), so none of this applies there.
