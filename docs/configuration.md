...
| Variable | Description |
|---|---|
| `YORISHIRO_QUEUE_KIND` | `Postgres` (default), `Sqlite`, or `Redis`. Booting with `Redis` needs the `worker_redis` Cargo feature compiled in (enabled in this workspace's `Cargo.toml`) or startup fails with "No queue provider feature was selected and compiled" |
| `QUEUE_URL` | The queue backend's own connection URI. Defaults to `DATABASE_URL` on the `Postgres` kind in `development.yaml` (matching this deployment's own default queue provider); `production.yaml` requires it explicitly with no default on every kind, matching that file's own no-silent-fallback convention |
...
`PUT /hosted/workspace/worker-class` pins one workspace's `EmbeddingSyncWorker` jobs to `tenant_private` or `official` compute instead of the shared pool every workspace uses by default.
