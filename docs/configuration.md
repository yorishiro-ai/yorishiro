...
A workspace stamped `unconfigured` (created with no embedding provider available at all) skips this check, since there is no real prior model to compare against.
...
| `YORISHIRO_EMBEDDING_MODEL` | OpenAI-compatible provider only. Model name sent in the `model` field of the embeddings request, and stamped onto a workspace at creation time as the model it was embedded with. Every provider stamps its own model this way; the local provider stamps the model it loaded regardless of this variable, and a workspace created with no embedding provider configured at all is stamped `unconfigured` |
