...
| `YSR_SNAPSHOT_RETENTION_DAYS` | How many days a batch migration stays undoable (default: `30`; `0` keeps every before-image forever). A migration writes one image per entity it touches, and only an undo takes them away, so an unbounded workspace that migrates repeatedly ends up holding more images than entities. The sweep runs at the start of the next migration in that workspace rather than on a timer. Undoing a job past the window answers `404`, the same as a job that never ran |
